...
local validateHwidAndKey = function(content)
    if content then
        local encrypted_ID, encrypted_Key, encrypted_start_time, encrypted_end_time = content:match("([^|]+)|([^|]+)|([^|]+)|([^|]+)")
        if encrypted_ID and encrypted_Key and encrypted_start_time and encrypted_end_time then
            ...
            return { ID = decrypted_ID, Key = decrypted_Key, Start = decrypted_start_time, End = decrypted_end_time }
        end
    end
    return nil
end
